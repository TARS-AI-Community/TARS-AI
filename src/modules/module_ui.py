...
class UIManager(threading.Thread):
    ...
    def cycle_background(self):
        ...
        # Don't update background_type here! It will be updated after new system is created

        self.background_change_requested = True
        # Don't save settings here - background_type hasn't been updated yet!
    ...
    def _init_background(self, bg_type):
        ...
        # Create the NEW background system first (keeps old one visible during creation)
        new_particle = None
        new_starfield = None
        new_tesseract = None
        new_video = None
        ...
        # Now swap to the new background (instant switch, no flicker)
        ...
        try:
            ...
            while self.running and not self.shutdown_event.is_set():
                ...
                if self.background_change_requested and self.next_background:
                    # Create new background FIRST, then update background_type
                    # This prevents black screen during initialization
                    self._init_background(self.next_background)
                    self.background_type = self.next_background  # Now safe to update
                    
                    # Save settings AFTER background_type is updated
                    self._save_ui_settings()
