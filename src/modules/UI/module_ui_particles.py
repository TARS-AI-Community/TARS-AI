...
class Particle:
    """Individual particle with gaussian blur effect"""
    ...
    def __init__(self, width, height):
        ...
        # Store original velocity for returning to normal mode
        self.original_vx = self.vx
        self.original_vy = self.vy
        
        # Animation properties
        self.energy_boost = random.uniform(2.0, 3.5)  # Speed multiplier
        self.pulse_offset = random.uniform(0, math.pi * 2)  # Random phase for pulsing
        ...
        self.base_alpha = self.alpha  # Store original alpha
        self.current_scale = 1.0  # Current size scale
        
        self.width = width
        self.height = height
        
        # Pre-render particle surfaces with gaussian blur
    ...
    def create_blurred_particle(self, mode='normal'):
        """Create a particle with proper gaussian blur using PIL"""
        # Make size large enough for blur
        ...
        # Color based on mode
        if mode == 'bright':
            # Brighter, more vivid blue/cyan for think mode
            color = (
        ...
        # Apply gaussian blur multiple times for softer effect
    ...
    def update(self, action_weight=0, think_weight=0, memory_weight=0, pulse_phase=0):
        """Update particle position with blended animation weights"""
        ...
        if think_weight > 0:
            ...
            # More intense pulsing and brighter
            ...
            # Grow particles during think mode
            think_scale = 1.0 + pulse_value * 0.4  # Grow up to 1.4x size
            target_scale += (think_scale - 1.0) * think_weight
        
        # Blend add_memory animation
        if memory_weight > 0:
        ...
        # Smoothly transition velocity, brightness, and scale
        ...
        # Set surface based on dominant animation
        if think_weight > 0.1:
    ...
    def draw(self, surface):
        ...
        # Apply brightness adjustment for pulsing
...
class ParticleSystem:
    """Manages a collection of bokeh-style particles"""
    ...
    def __init__(self, width, height, num_particles=250, bg_color=(0, 0, 0)):
        ...
        # Animation weights (0 to 1) - can all be active simultaneously
    ...
    def add_memory(self):
        """Activate memory formation animation for 2 seconds"""
        self.memory_end_time = time.time() + 2.0
    
    def think(self):
        """Activate thinking animation mode for 5 seconds (bright particles that grow)"""
    ...
    def update(self):
        ...
        # Update pulse phase if any animation is active
        if self.action_weight > 0 or self.think_weight > 0 or self.memory_weight > 0:
            self.pulse_phase += 0.1
        
        # Update particles with blended weights
    ...
    def draw(self, surface):
        ...
        # Draw connection lines for add_memory mode (scaled by weight)
        if self.memory_weight > 0.1:
            self.draw_memory_connections(surface)
    
    def draw_memory_connections(self, surface):
        """Draw connecting lines between nearby particles to simulate memory formation"""
        short_distance = 120
        long_distance = 300  # Some connections reach farther
        
        # Pulse the connection intensity
        pulse_intensity = (math.sin(self.pulse_phase * 3) + 1) / 2
        base_alpha = int(80 * pulse_intensity * self.memory_weight)  # Scale by weight
        
        connections_drawn = 0
        max_connections = 150  # Limit for performance
        ...
        for i, p1 in enumerate(self.particles):
            ...
            for p2 in self.particles[i+1:]:
                ...
                # Connect particles within extended range
                if dist < long_distance:
                    ...
                    if alpha > 10:
                        ...
                        # Create line surface for anti-aliasing
